...
def test_dashboard_configures_and_controls_voice_without_exposing_key(tmp_path):
    async def scenario():
        ...
        original, transport, missions, events, manager, actions, operator = service_parts
        created = []
        ...
        await gateway.execute("dashboard-token-secure", "configure_voice", {
            "api_key": api_key,
        ...
        serialized = json.dumps({"snapshot": snapshot, "events": [
            event.detail for event in events.replay()]})
...
def test_dashboard_voice_configuration_is_validated(tmp_path):
    ...
    with pytest.raises(ValueError, match="length"):
        asyncio.run(gateway.execute("dashboard-token-secure", "configure_voice", {
            "api_key": "short",
        }))
