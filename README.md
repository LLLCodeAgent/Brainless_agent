...
Runtime agent and action records are incrementally bridged into correlated dashboard events. The Analytics page uses real terminal mission/task, action audit, retry, intervention, agent failure, and approval records. A rate displays `N/A` when no valid denominator exists rather than inventing a score.
